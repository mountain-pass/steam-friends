...
API.getGames = steamId => get(`http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${steamApiKey}&steamid=${steamId}&include_appinfo=1&include_played_free_games=1`)
    ...
    .get((req, res) => {
        ...
        Promise.all(promises)
            .then(responses => {
                    ...
                    // games
                    Object.keys(gamesByAppId).sort((a, b) => countFn(b) - countFn(a)).forEach(appid => {
                        ...
                        res.write(friendIds.map(id => `<th class="${(owners.indexOf(id) !== -1 ? 'green' : '')}">` + (owners.indexOf(id) !== -1 ? 'TRUE' : '') + `</th>`).join(''));
                        res.write('</tr>');
                    });
