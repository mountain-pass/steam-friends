...
app.route('/friends').get((req, res) => {
  var username = req.query.username
  API.getVanityUrl(username).then(
    (vanityRes) => {
              ...
              res.write(`
                                ...
                                ${friends
                                  .map(
                                    (f) => `
                                    <div card>
                                        <input type="checkbox" name="friendIds" value="${f.steamid}" ${
                                      f.steamid === mySteamId ? 'checked' : ''
                                    } />
                                        <div card-body>
                                            ...
                                            <div><label>SteamId:</label><span><a href="${
                                              f.profileurl
                                            }" target="_blank">${f.steamid || '-'}</a></span></div>
                                        </div>
                                    </div>
                                `
                                  )
...
app.route('/friends/games').get((req, res) => {
      ...
      res.write(`
                    ...
                    ${Object.keys(gamesByAppId)
                      ...
                      .map((game) => {
                        return `
                            <tr>
                                <th><img src="http://media.steampowered.com/steamcommunity/public/images/apps/${
                                  game.appid
                                }/${game.img_logo_url}.jpg" /></th>
                                <th>${game.name}</th>
                                    ...
                                    if (userGame) {
                                      if (userGame.playtime_forever > 0) {
                                        const userPercentage = Math.round(
                                          (userGame.playtime_forever / f.game_minutes) * 100
                                        )
                                        const gamePercentage = Math.round(
                                          (userGame.playtime_forever / game.playtime_forever) * 100
                                        )
                                        return `
                                                <th class="green ${
                                                  userPercentage < 2.5
